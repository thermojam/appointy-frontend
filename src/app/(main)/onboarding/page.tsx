...
const OnboardingLayout = ({ title, subtitle, step, totalSteps, children, onNext, onBack, isNextDisabled, isPending }: OnboardingLayoutProps) => {
    return (
        <div className="flex min-h-screen w-full flex-col items-center justify-center bg-background p-4">
            <div className="w-full max-w-md">

                <div className="w-full rounded-[40px] border bg-surface border-border shadow-xl p-8">
                    <main>
                    ...
                    <footer className="mt-10 flex justify-between items-center">
                        <button onClick={onBack} disabled={step === 1 || isPending} className="text-sm font-medium transition-colors hover:text-primary disabled:opacity-50 disabled:cursor-not-allowed">
                            &lt; Назад
                        </button>
                        <button
                            onClick={onNext}
    ...
    const handleStepValid = useCallback((data: StepData) => {
        ...
        if (step < totalSteps) {
            ...
            if (role === 'master') {
                ...
                console.log('СИМУЛЯЦИЯ: Сохранение клиента', newFormData);
                setTimeout(() => setStep(prev => prev + 1), 1000);
            }
        }
    }, [formData, step, totalSteps, role, updateMasterProfile]); // Добавляем все зависимости

    const steps = useMemo(() => ({
        master: {
            1: { title: 'Основная информация', subtitle: 'Эта информация будет видна клиентам', component: <Step1_BaseInfo ref={step1Ref} role={role} onValid={handleStepValid} initialData={formData} /> },
            2: { title: 'Место работы', subtitle: 'Расскажите, где клиенты могут найти Вас.', component: <Step2_Workplace ref={step2MasterRef} onValid={handleStepValid} initialData={formData} /> },
            3: { title: 'О себе', subtitle: 'Расскажите подробную информацию о своих профессиональных качествах.', component: <Step3_About ref={step3MasterRef} onValid={handleStepValid} initialData={formData} /> },
            4: { title: 'Правила бронирования', subtitle: 'Настройте, как клиенты могут записываться и отменять прием.', component: <Step4_BookingRules ref={step4MasterRef} onValid={handleStepValid} initialData={formData} /> },
        },
        client: {
            1: { title: 'Основная информация', subtitle: 'Эта информация будет видна клиентам', component: <Step1_BaseInfo ref={step1Ref} role={role} onValid={handleStepValid} initialData={formData} /> },
            2: { title: 'В чем Вы заинтересованы?', subtitle: 'Выберите услуги, которые вы хотели бы попробовать', component: <Step2_Interests ref={step2ClientRef} onValid={handleStepValid} initialData={formData} /> },
        }
        // ИЗМЕНЕНИЕ: Добавляем handleStepValid в массив зависимостей
    }), [role, handleStepValid, formData]);

    if (step > totalSteps) {
        return <OnboardingCompletion role={role} onBack={() => setStep(totalSteps)} />;
    }
